...
#ifdef USE_CUDA
template<>
void SassConvBatchnormScale<NV, Precision::FP32>::operator()(
    OpContext<NV>& ctx,
...
Status SassConvBatchnormScaleHelper<Ttype, Ptype>::InitParam() {
	...
	if(check_attr("merge_type")) {
    	...
    	ActivationParam<Tensor4d<Ttype>> activation_param(Active_relu);
    	...
    	EltwiseActiveParam<Tensor4d<Ttype>> eltwise_relu_param(eltwise_param, activation_param);

		ConvActiveParam<Tensor4d<Ttype>> conv_act_param(_conv_param, batchnorm_param, scale_param, eltwise_relu_param);
		_param_conv_batchnorm_scale_relu = conv_act_param;
	} else { 
		ConvActiveParam<Tensor4d<Ttype>> conv_act_param(_conv_param, batchnorm_param, scale_param); 
		_param_conv_batchnorm_scale_relu = conv_act_param;
	}
