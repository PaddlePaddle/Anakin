...
#ifdef USE_ARM_PLACE
template<>
void ConvBatchnormScale<ARM, AK_FLOAT, Precision::FP32>::operator()(
    OpContext<ARM>& ctx,
    const std::vector<Tensor4dPtr<ARM, AK_FLOAT> >& ins,
    std::vector<Tensor4dPtr<ARM, AK_FLOAT> >& outs) {
    auto* impl = static_cast<ConvBatchnormScaleHelper<ARM, AK_FLOAT, Precision::FP32>*>(this->_helper);
    auto& param = static_cast<ConvBatchnormScaleHelper<ARM, AK_FLOAT, Precision::FP32>*>
                  (this->_helper)->_param_conv_batchnorm_scale;
    impl->_funcs_conv_batchnorm_scale(ins, outs, param, ctx);
}
#endif
