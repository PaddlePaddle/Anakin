...
#ifdef USE_ARM_PLACE
template<>
void Im2Sequence<ARM, AK_FLOAT, Precision::FP32>::operator() (OpContext<ARM> &ctx, 
                          const std::vector<Tensor4dPtr<ARM, AK_FLOAT> >& ins, 
                          std::vector<Tensor4dPtr<ARM, AK_FLOAT> >& outs) {
    auto* impl = static_cast<Im2SequenceHelper<ARM, AK_FLOAT, Precision::FP32>*>(this->_helper);
    auto& param = static_cast<Im2SequenceHelper<ARM, AK_FLOAT, Precision::FP32>*>(this->_helper)->_param_im2sequence;
    impl->_funcs_im2sequence(ins, outs, param, ctx);
}
#endif
