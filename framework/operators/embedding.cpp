...
#ifdef USE_ARM_PLACE
template<>
void Embedding<ARM, AK_FLOAT, Precision::FP32>::operator()(
    OpContext<ARM>& ctx,
    const std::vector<Tensor4dPtr<ARM, AK_FLOAT> >& ins,
    std::vector<Tensor4dPtr<ARM, AK_FLOAT> >& outs) {
    auto* impl =
        static_cast<EmbeddingHelper<ARM, AK_FLOAT, Precision::FP32>*>(this->_helper);
    auto& param =
        static_cast<EmbeddingHelper<ARM, AK_FLOAT, Precision::FP32>*>(this->_helper)->_param_embedding;
    impl->_funcs_embedding(ins, outs, param, ctx);
}
#endif
