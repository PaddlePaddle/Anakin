...
Status LrnHelper<Ttype, Ptype>::InitParam() {
    ...
    if (norm_region_in == "ACROSS_CHANNELS") {
        LrnParam<Tensor4d<Ttype>> param_lrn(local_size_in, alpha_in, beta_in, k_in, ACROSS_CHANNELS);
        _param_lrn = param_lrn;
    } else if (norm_region_in == "WITHIN_CHANNEL") {
        LrnParam<Tensor4d<Ttype>> param_lrn(local_size_in, alpha_in, beta_in, k_in, WITHIN_CHANNEL);
        _param_lrn = param_lrn;
    } else {
