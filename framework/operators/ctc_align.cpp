...
#ifdef USE_ARM_PLACE
template<>
void CtcAlign<ARM, AK_FLOAT, Precision::FP32>::operator() (OpContext<ARM> &ctx,
                          const std::vector<Tensor4dPtr<ARM, AK_FLOAT> >& ins,
                          std::vector<Tensor4dPtr<ARM, AK_FLOAT> >& outs) {
    auto* impl = static_cast<CtcAlignHelper<ARM, AK_FLOAT, Precision::FP32>*>(this->_helper);
    auto& param = static_cast<CtcAlignHelper<ARM, AK_FLOAT, Precision::FP32>*>(this->_helper)->_param_ctc_align;
    impl->_funcs_ctc_align(ins, outs, param, ctx);
}
#endif
