#include "framework/operators/rcnn_proposal.h"
namespace anakin {
namespace ops {
#ifdef USE_CUDA
template<>
void RCNNProposal<NV,  Precision::FP32>::operator()(
    OpContext<NV>& ctx,
    const std::vector<Tensor4dPtr<NV> >& ins,
    std::vector<Tensor4dPtr<NV> >& outs) {
    auto* impl = static_cast<RCNNProposalHelper<NV,  Precision::FP32>*>(this->_helper);
    auto& param = static_cast<RCNNProposalHelper<NV,  Precision::FP32>*>
                  (this->_helper)->_param__rcnn_prop;
    impl->_funcs_rcnn_prop(ins, outs, param, ctx);
}
#endif
/// TODO ... specialization other type of operator
/// set helper
template<typename Ttype,Precision Ptype>
RCNNProposalHelper<Ttype, Ptype>::~RCNNProposalHelper() {
}
template<typename Ttype,Precision Ptype>
Status RCNNProposalHelper<Ttype, Ptype>::InitParam() {
    LOG(WARNING) << "Parsing RCNNProposal op parameter.";
    // get nms_param
    ...
    detection_output_ssd_param.has_param = true;
    ...
    _param__rcnn_prop = proposal_param;
    return Status::OK();
}
template<typename Ttype,Precision Ptype>
Status RCNNProposalHelper<Ttype, Ptype>::Init(OpContext<Ttype>& ctx,
        const std::vector<Tensor4dPtr<Ttype> >& ins,
        std::vector<Tensor4dPtr<Ttype> >& outs) {
    _funcs_rcnn_prop.init(ins, outs, _param__rcnn_prop, SPECIFY, SABER_IMPL, ctx);
    return Status::OK();
}
template<typename Ttype,Precision Ptype>
Status RCNNProposalHelper<Ttype, Ptype>::InferShape(const
        std::vector<Tensor4dPtr<Ttype> >& ins,
        std::vector<Tensor4dPtr<Ttype> >& outs) {
    _funcs_rcnn_prop.compute_output_shape(ins, outs,  _param__rcnn_prop);
    return Status::OK();
}
#ifdef USE_CUDA
template class RCNNProposalHelper<NV,  Precision::FP32>;
template class RCNNProposalHelper<NV,  Precision::FP16>;
template class RCNNProposalHelper<NV,  Precision::INT8>;
#endif
#ifdef USE_ARM_PLACE
template class RCNNProposalHelper<ARM,  Precision::FP32>;
template class RCNNProposalHelper<ARM,  Precision::FP16>;
template class RCNNProposalHelper<ARM,  Precision::INT8>;
#endif
// register helper
#ifdef USE_CUDA
ANAKIN_REGISTER_OP_HELPER(RCNNProposal, RCNNProposalHelper, NV,  Precision::FP32);
#endif
#ifdef USE_ARM_PLACE
ANAKIN_REGISTER_OP_HELPER(RCNNProposal, RCNNProposalHelper, ARM,  Precision::FP32);
#endif
//! register op
ANAKIN_REGISTER_OP(RCNNProposal)
.Doc("RCNNProposal operator")
#ifdef USE_CUDA
