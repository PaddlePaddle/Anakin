...
#ifdef USE_ARM_PLACE
template<>
void Reshape<ARM, AK_FLOAT, Precision::FP32>::operator()(
    OpContext<ARM>& ctx,
    const std::vector<Tensor4dPtr<ARM, AK_FLOAT> >& ins,
    std::vector<Tensor4dPtr<ARM, AK_FLOAT> >& outs) {
    auto* impl =
        static_cast<ReshapeHelper<ARM, AK_FLOAT, Precision::FP32>*>(this->_helper);
    auto& param =
        static_cast<ReshapeHelper<ARM, AK_FLOAT, Precision::FP32>*>(this->_helper)->_param_reshape;
    impl->_funcs_reshape(ins, outs, param, ctx);
}
#endif
