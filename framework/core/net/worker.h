...
public:
     ...
     */
    std::vector<Tensor4dPtr<Ttype> > sync_prediction(\
        std::vector<Tensor4dPtr<typename target_host<Ttype>::type> >& net_in_list);
     ...
     *  \return the net graph outputs.
     */
    std::vector<Tensor4dPtr<Ttype> > sync_prediction_device(\
