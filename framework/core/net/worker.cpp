...
struct NetGraphWrapper {
    ...
    void initial(std::string model_path,
                 std::unordered_map<std::string, std::vector<int>>& shape_map) EXCLUSIVE_LOCKS_REQUIRED(this->_mut) {
...
template<typename Ttype, Precision Ptype, OpRunType RunType>
Worker<Ttype, Ptype, RunType>::Worker(std::string model_path,
                                      int num_thread) : _model_path(model_path), ThreadPool(num_thread) {}
...
Worker<Ttype, Ptype, RunType>::sync_prediction(
    std::vector<Tensor4d<typename target_host<Ttype>::type> >& net_ins_list) {
...
template<typename Ttype, Precision Ptype, OpRunType RunType>
std::future<std::vector<Tensor4dPtr<Ttype> > >
Worker<Ttype, Ptype, RunType>::sync_prediction_device(std::vector<Tensor4dPtr<Ttype> >&
        net_ins_list) {
...
template<typename Ttype, Precision Ptype, OpRunType RunType>
void Worker<Ttype, Ptype, RunType>::async_prediction(
    std::vector<Tensor4dPtr<typename target_host<Ttype>::type> >& net_ins_list) {
    std::lock_guard<std::mutex> guard(this->_async_que_mut);
    auto task = [&](std::vector<Tensor4dPtr<typename target_host<Ttype>::type> >& ins) ->
