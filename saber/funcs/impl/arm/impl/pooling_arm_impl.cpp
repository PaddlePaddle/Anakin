...
void pooling3x3s2_max_0(Tensor<ARM, AK_FLOAT, NCHW>& tensor_out, \
    Tensor<ARM, AK_FLOAT, NCHW>& tensor_in, \
    int kernel_w, int kernel_h, int stride_w, int stride_h, int pad_w, int pad_h);

void pooling3x3s2_max_1(Tensor<ARM, AK_FLOAT, NCHW>& tensor_out, \
    Tensor<ARM, AK_FLOAT, NCHW>& tensor_in, \
    int kernel_w, int kernel_h, int stride_w, int stride_h, int pad_w, int pad_h);
...
void pooling3x3s2_max(Tensor<ARM, AK_FLOAT, NCHW>& tensor_out, \
    ...
    const float* data_in = tensor_in.data();
    if(pad_w == 0){
        pooling3x3s2_max_0(tensor_out, tensor_in, kernel_w, kernel_h, stride_w, stride_h, pad_w, pad_h);
    }else if(pad_w == 1){
        pooling3x3s2_max_1(tensor_out, tensor_in, kernel_w, kernel_h, stride_w, stride_h, pad_w, pad_h);
    }else{
        pooling_basic(tensor_out,tensor_in,type, global, kernel_w, kernel_h, stride_w, stride_h, pad_w, pad_h);
    }
            ...
            for (; h < h_even; h += 2) {
               ...
               // data_out_channel[0] = std::max(std::max(maxr0, maxr1), maxr2);
                ...
                if (cnt_num > 0 || cnt_num1 > 0){
                    ...
                    "ble       loop3_p0                                  @ble exit\n"
                    "s3_max_loop_mid_p0:                                @main loop\n"
                    ...
                    "subs      %[cnt_num], #1                            @subs cnt_num, #1\n"
                    "bne       s3_max_loop_mid_p0                       @bne s3_max_loop_mid\n"
                    "loop3_p0:                                       @loop \n"
                    "cmp       %[cnt_num1], #0                           @cmp cnt_num, 0\n"
                    "ble       exit1_p0                                 @ble exit1\n"
                    "s3_max_loop_mid_1_p0:                              @mid loop\n"
                    ...
                    "bne       s3_max_loop_mid_1_p0                     @bne s3_max_loop_mid_1\n"
                    "exit1_p0:                                           @exit\n"
                    :[dr0] "+r" (dr0), [dr1] "+r" (dr1), [dr2] "+r" (dr2), [dr_out] "+r" (dr_out), \
               ...
               // int hend = std::min(std::min(hstart + kernel_h, h_in + pad_h),h_in);
                ...
                for(; w < w_even - 1; w += 2){
                    ...
                    if (cnt_num > 0 || cnt_num1 > 0){
                        asm volatile(
                        "cmp       %[cnt_num], #0                        @cmp cnt_num, 0\n"
                        "ble       loop5_p0                                  @ble exit\n"
                        "s3_max_loop_bot1_p0:                               @main loop\n"
                        "vld1.f32  {d0-d3}, [%[dr0]]!                     @load d0-d5, dr0\n"
                        "vld1.f32  {d6-d9}, [%[dr1]]!                    @load d4-d7, dr1\n"
                        ...
                        "vst1.f32  d9, [%[dr_out]]!                      @vst1 d0, dr_out\n"
                        "subs      %[cnt_num], #1                            @subs cnt_num, #1\n"
                        "bne       s3_max_loop_bot1_p0                      @bne s3_max_loop_bot\n"
                        "loop5_p0:                                      @loop \n"
                        "cmp       %[cnt_num1], #0                           @cmp cnt_num, 0\n"
                        "ble       exit4_p0                                 @ble exit\n"
                        "s3_max_loop_bot1_1_p0:                             @bot loop\n"
                        "vld1.f32  {d0-d1}, [%[dr0]]!                     @load d0-d1, dr0\n"
                        "vld1.f32  {d2-d3}, [%[dr1]]!                     @load d2-d3, dr1\n"
                        "vmov.f32  s3,s2                                 @movs3, s2\n"
                        "vmov.f32  s7,s6                                 @movs7, s6\n"
                        ...
                        "sub       %[dr1], #8                            @add w, 6\n"
                        "subs      %[cnt_num1], #1                           @subs cnt_num, #1\n"
                        "bne       s3_max_loop_bot1_1_p0                    @bne s3_max_loop_bot_1\n"
                        "exit4_p0:                                          @exit\n"
...
void pooling3x3s2_ave_0(Tensor<ARM, AK_FLOAT, NCHW>& tensor_out, \
    Tensor<ARM, AK_FLOAT, NCHW>& tensor_in, \
    int kernel_w, int kernel_h, int stride_w, int stride_h, int pad_w, int pad_h);

void pooling3x3s2_ave_1(Tensor<ARM, AK_FLOAT, NCHW>& tensor_out, \
    Tensor<ARM, AK_FLOAT, NCHW>& tensor_in, \
    int kernel_w, int kernel_h, int stride_w, int stride_h, int pad_w, int pad_h);
...
void pooling3x3s2_ave(Tensor<ARM, AK_FLOAT, NCHW>& tensor_out, \
    ...
    const float* data_in = tensor_in.data();
    if(pad_w == 0){
        pooling3x3s2_ave_0(tensor_out, tensor_in, kernel_w, kernel_h, stride_w, stride_h, pad_w, pad_h);
    }else if(pad_w == 1){
        pooling3x3s2_ave_1(tensor_out, tensor_in, kernel_w, kernel_h, stride_w, stride_h, pad_w, pad_h);
    }else{
        pooling_basic(tensor_out,tensor_in,type, global, kernel_w, kernel_h, stride_w, stride_h, pad_w, pad_h);
    }
}
                ...
                // data_out_channel[0] =(r0[0] + r0[1] + r1[0] + r1[1]) / 9.f;
                 ...
                 for(; w < w_even - 1; w += 2){
                    ...
                    if (cnt_num > 0 || cnt_num1 > 0){
                        asm volatile(
                        "cmp       %[cnt_num], #0                        @cmp cnt_num, 0\n"
                        "ble       loop5_ave_p0                                  @ble exit\n"
                        "s3_ave_loop_bot1_p0:                               @main loop\n"
                        "vld1.f32  {d0-d3}, [%[dr0]]!                     @load d0-d5, dr0\n"
                        "vld1.f32  {d6-d9}, [%[dr1]]!                    @load d4-d7, dr1\n"
                        "vld1.f32  {d4}, [%[dr0]]!                     @load d0-d3, dr0\n"
                        "vld1.f32  {d10}, [%[dr1]]!                  @load d4-d7, dr1\n"
                        ...
                        "vst1.f32  d8, [%[dr_out]]!                      @vst1 d0, dr_out\n"
                        "vst1.f32  d9, [%[dr_out]]!                      @vst1 d0, dr_out\n"
                        "bne       s3_ave_loop_bot1_p0                      @bne s3_max_loop_bot\n"
                        "loop5_ave_p0:                                      @loop \n"
                        "cmp       %[cnt_num1], #0                           @cmp cnt_num, 0\n"
                        "ble       exit4_ave_p0                                 @ble exit\n"
                        "s3_ave_loop_bot1_1_p0:                             @bot loop\n"
                        "vld1.f32  {d0-d1}, [%[dr0]]!                     @load d0-d1, dr0\n"
                        "vld1.f32  {d2-d3}, [%[dr1]]!                     @load d2-d3, dr1\n"
                        "vext.f32  q0, %q[vzero], q0, #3                 @ ext v0_0123\n"
                        "vext.f32  q1, %q[vzero], q1, #3                 @ ext v1_0123\n"
                        ...
                        "subs      %[cnt_num1], #1                           @subs cnt_num, #1\n"
                        "vst1.f32  d0[0], [%[dr_out]]!                   @vst  d0[0], dr_out\n"
                        "bne       s3_ave_loop_bot1_1_p0                    @bne s3_max_loop_bot_1\n"
                        "exit4_ave_p0:                                          @exit\n"
                        :[dr0] "+r" (dr0), [dr1] "+r" (dr1), [dr_out] "+r" (dr_out), [cnt_num] "+r" (cnt_num), \ 
