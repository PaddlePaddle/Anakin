...
create(const std::vector<OpTensor*>& inputs,
    ...
    std::vector<std::vector<int>> xdim(batch_size);
    std::vector<std::vector<int>> xstride(batch_size);

    for (int i = 0; i < batch_size; i++) {
        int seq_len = offset[i + 1] - offset[i];
        xdim[i] = {seq_len, _word_size, 1};
        xstride[i] = {_word_size, 1, 1};
    }

    _xDesc.reset(new cudnn::TensorDescriptors<OpDataType>(batch_size, xdim, xstride));

    std::vector<std::vector<int>> ydim(batch_size);
    std::vector<std::vector<int>> ystride(batch_size);

    for (int i = 0; i < batch_size; i++) {
        int seq_len = offset[i + 1] - offset[i];
        ydim[i] = {seq_len, _hidden_size * param.num_direction, 1};
        ystride[i] = {_hidden_size * param.num_direction, 1, 1};
    }

    _yDesc.reset(new cudnn::TensorDescriptors<OpDataType>(batch_size, ydim, ystride));
    ...
    int max_seq_len = offset[1] - offset[0];
    ...
    //FIXME tryEXP
    LOG(INFO) << "work space =  " << _workspace_size_in_bytes << ",batch size = " << batch_size <<
              ",max = " << max_seq_len;
...
void VenderGru<NV, AK_FLOAT>::trans_akweights_2_cudnnweights(GruParam<NV>& param) {
    int weights_i2h_size = _hidden_size * _word_size;
...
init(const std::vector<OpTensor*>& inputs,
    ...
    LOG(INFO) << "w:" << _word_size << "," << _hidden_size;
    ...
    LOG(INFO) << "::" << "," << weightsSize << "," << cudnn::cudnnTypeWrapper<OpDataType >::type;
    Shape weight_tensor_shape({1, 1, 1, weightsSize / sizeof(OpDataType)});
    _inner_weight.re_alloc(weight_tensor_shape, AK_FLOAT);

    CUDNN_CHECK(cudnnSetFilterNdDescriptor(
                    _wDesc, cudnn::cudnnTypeWrapper<OpDataType >::type, CUDNN_TENSOR_NCHW, 3, dims));
    /**
