...
//}
template <typename Dtype>
void  get_sub_tensor(const Dtype* in, Dtype* out, int h, int w, int stride_w) {
    int num_threads = h * w;
    sub_tensor<<<CUDA_GET_BLOCKS(num_threads), CUDA_NUM_THREADS, 0, 0>>>(in, out,  h, w, stride_w);
}

template  void get_sub_tensor(const float* in, float* out, int h, int w, int stride_w);
