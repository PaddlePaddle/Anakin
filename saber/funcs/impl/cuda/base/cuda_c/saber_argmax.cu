    ...
    if (param.has_axis) {
        ...
        if (param.top_k == 1) {
            if (inner_dim / CUDA_NUM_THREADS < 10) {
                ...
                top1<OpDataType, CUDA_NUM_THREADS><<<outer_dim, CUDA_NUM_THREADS, 0, cuda_stream>>>((const OpDataType*)_block_max_value.data(), (const OpDataType*)_block_max_index.data(), outer_dim, block_num, param.out_max_val, out_data);
            }
        } else {
