...
void seq_pool_average(dtype* dst, const dtype* src_in,const int batch_size,
    ...
    seq_pool_average_kernel<<<CUDA_GET_BLOCKS(batch_size * slice_size),CUDA_NUM_THREADS>>>\
...
void seq_pool_sum(dtype* dst, const dtype* src_in,const int batch_size,
                      const int* seq_offset, const int slice_size, Context<NV>* ctx) {
    seq_pool_sum_kernel<<<CUDA_GET_BLOCKS(batch_size * slice_size),CUDA_NUM_THREADS>>>\
        (dst,src_in,batch_size,seq_offset,slice_size);
}
...
void seq_pool_sqrt(dtype* dst, const dtype* src_in,const int batch_size,
                      const int* seq_offset, const int slice_size, Context<NV>* ctx) {
    seq_pool_sqrt_kernel<<<CUDA_GET_BLOCKS(batch_size * slice_size),CUDA_NUM_THREADS>>>\
        (dst,src_in,batch_size,seq_offset,slice_size);
}
...
void seq_pool_max(dtype* dst, const dtype* src_in,const int batch_size,
                      const int* seq_offset, const int slice_size, Context<NV>* ctx) {
    seq_pool_max_kernel<<<CUDA_GET_BLOCKS(batch_size * slice_size),CUDA_NUM_THREADS>>>\
        (dst,src_in,batch_size,seq_offset,slice_size);
}
...
void seq_pool_first(dtype* dst, const dtype* src_in,const int batch_size,
    ...
    for (int i = 0; i < batch_size; ++i){
        int in_slice_num = seq_offset[i];
        CUDA_CHECK(cudaMemcpyAsync(dst + i * slice_size, src_in + in_slice_num * slice_size, \
        sizeof(dtype) * slice_size, cudaMemcpyDeviceToDevice, ctx->get_compute_stream()));
    }
}
...
void seq_pool_last(dtype* dst, const dtype* src_in,const int batch_size,
                      const int* seq_offset, const int slice_size, Context<NV>* ctx) {
    for (int i = 0; i < batch_size; ++i){
        int in_slice_num = seq_offset[i+1];
        CUDA_CHECK(cudaMemcpyAsync(dst + i * slice_size, src_in + (in_slice_num-1) * slice_size, \
        sizeof(dtype) * slice_size, cudaMemcpyDeviceToDevice, ctx->get_compute_stream()));
    }
}
...
SaberStatus SaberSequencePool<NV, OpDtype>::dispatch(
    ...
    if(param.sequence_pool_type == Sequence_pool_first || param.sequence_pool_type == Sequence_pool_last){
	   kernel_direct_map[param.sequence_pool_type](dst_ptr, src_ptr, batch_size, (const int*)seq_offset.data(), slice_size, this->_ctx);
    }
    else{
