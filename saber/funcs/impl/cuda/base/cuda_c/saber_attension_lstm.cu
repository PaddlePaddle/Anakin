...
__global__ void lstm_bias_and_act(const Dtype* in_data, const Dtype* bias_data, Dtype* out_data, Dtype* cell_data, int batch_size, int hidden_size) {
    ...
    Dtype ct  = tanh(tmp_in[0] + tmp_bias[0]);
    Dtype ig  = sigmoid(tmp_in[hidden_size] + tmp_bias[hidden_size]);
    Dtype fg  = sigmoid(tmp_in[2 * hidden_size] + tmp_bias[2 * hidden_size]);
...
template <typename Dtype>
__global__ void sequence_bias(const Dtype* in_data, const Dtype* bias_data, const int * seq_id, const int num, const int dim,
                 Dtype* out_data) {
    int tid = threadIdx.x + blockIdx.x * blockDim.x;
    if (tid > dim * num) {
        return;
    ...
    out_data[tid] = in_data[tid] + bias_data[cur_seq_id * dim + dim_id];
}
...
template<>
    ...
    for (int i = 0; i < word_num; i++) {
        if (i < seq_offset[seq_id + 1]) {
            id_map.push_back(seq_id);
        } else {
            seq_id++;
            i -= 1;
        }
    ...
    for (int i = 0; i < seq_num; i++) {
        int cur_len = seq_offset[seq_id + 1] - seq_offset[seq_id];
    ...
    DataTensor_out lstm_out;
    Shape lstm_out_shape = {max_len * seq_num, hidden_size, 1, 1};
    lstm_out.reshape(lstm_out_shape);
    ...
    for (int word_id = 0; word_id < max_len; word_id++) {
        ...
        if (word_id > 0) {
            ...
            kernel_1(seq_num, N_0, hidden_size, 1.0f, lstm_out.data() + (word_id - 1) * seq_num * hidden_size, 0.f, 
                fc_vec[0]->weights->data() + K_0 * N_0,  _first_fc_out_1.mutable_data(), stream);
            sequence_bias<<<CUDA_GET_BLOCKS(_attn_outs[0]->valid_size()), CUDA_NUM_THREADS, 0, stream>>>(_first_fc_out_0.data(), _first_fc_out_1.data(), 
        ...
        relu<<<CUDA_GET_BLOCKS(_attn_outs[0]->valid_size()), CUDA_NUM_THREADS, 0, stream>>>(data_out, data_out, _attn_outs[0]->valid_size());
        ...
        for (int i = 1; i < attn_param.fc_vec.size(); i++) {
            ...
            relu<<<CUDA_GET_BLOCKS(_attn_outs[i]->valid_size()), CUDA_NUM_THREADS, 0, stream>>>(fc_out_data, fc_out_data, _attn_outs[i]->valid_size());
        }
        
        Shape offset_shape = {seq_num + 1, 1, 1, 1};
        _dev_offset.reshape(offset_shape);
        cudaMemcpyAsync((void*)_dev_offset.mutable_data(), (void*)&seq_offset[0], sizeof(int) * seq_offset.size(),
        ...
        _gemm_wh(seq_num, 4 * hidden_size, hidden_size, 1.0, x_data, 1.0, _wh_data, _hidden_out.mutable_data(), stream);
