...
#define BARRIER_MARK_INIT (0x7fffffff)
...
#define OUT_WI_ROW_START (lid_x >> 4)
#define OUT_WI_COL_START (lid_x & 15)
#define LDS_WEI_ROW_START (lid_x >> 4)
#define LDS_IN_ROW_START (lid_x >> 4)
#define OFFSET ((grid_x % COUNTER_STRIDE) * 16 % QSTRIDE)
...
__attribute__((reqd_work_group_size(256, 1, 1))) __kernel void conv1x1_act(
    ...
    if (grid_x < COUNTER_STRIDE) {
        if (lid_x == 0) {
            *(pBarrier + COUNTER_INDEX) = BARRIER_MARK_INIT;
