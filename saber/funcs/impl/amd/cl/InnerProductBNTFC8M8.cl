    ...
    __global float *c)
{
	...
	for (int i = 0; i < ITER; i++, offset = (offset + 64) % STRIDE)
	{
		...
		for (int j = 0; j < 8; j++)
		{
			shared_b[j][(lid_x)] = (offset + (j + ((grid_x & 127) << 3)) * STRIDE + (lid_x) < OUTPUT * STRIDE ? pB[offset + (j + ((grid_x & 127) << 3)) * STRIDE + (lid_x)] : 0.0f); //correct
		}

		for (int k = 0; k < 8; k++)
		{
#if 1
			sum = mad(shared_a[((lid_x & 7) << 3) + k], shared_b[((lid_x) >> 3)][((lid_x & 7) << 3) + k], sum); //correct
#else
			sum += shared_a[((lid_x & 7) << 3) + k] * shared_b[((lid_x) >> 3)][((lid_x & 7) << 3) + k]; //correct
#endif
		}
	}
