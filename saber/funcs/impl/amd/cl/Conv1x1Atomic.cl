...
#define LDS_WEI_ROW_START ((lid_x / PER_ITER_STRIDE) * LDS_WEI_READ_ITER)
#define LDS_IN_ROW_START ((lid_x / PER_ITER_STRIDE) * LDS_IN_READ_ITER)
...
__attribute__((reqd_work_group_size(256, 1, 1))) __kernel void conv1x1_act(
    ...
    for (uchar k = 0; k < ITER; k++, offset = (offset + PER_ITER_STRIDE) % QSTRIDE) {
        for (uchar i = 0; i < LDS_WEI_READ_ITER; i++) {
            shared_wei
                    [WEI_WI_COL_START * LDS_WEI_STRIDE + (LDS_WEI_ROW_START + i)
                     + (LDS_WEI_ROW_START + i) / 32] =
                            pWei[(LDS_WEI_ROW_START + i) * STRIDE_WEI + offset];
        }
        ...
        for (uint n = 0; n < N; n++) {
            ...
            for (uchar j = 0; j < PER_ITER_STRIDE; j++) {
                for (uchar l = 0; l < PER_WI_TILE_COL; l++)
                    for (uchar m = 0; m < PER_WI_TILE_ROW; m++) {
                        sum[n][m][l] += pShared_wei
                                                [j * LDS_WEI_STRIDE + (OUT_WI_ROW_START + m)
                                                 + (OUT_WI_ROW_START + m) / 32]
                                        * pShared_in[j + (OUT_WI_COL_START + l) * LDS_IN_STRIDE];
                    }
            }
            barrier(CLK_LOCAL_MEM_FENCE);
            ...
            for (uint n = 0; n < N; n++)
                for (uint j = 0; j < PER_WI_TILE_ROW; j++)
                    for (uint i = 0; i < PER_WI_TILE_COL; i++) {
                        *(pOut + j * STRIDE_IN + i + n * OUT_BATCH_STRIDE) =
                                sum[n][j][i] + pBias[j];
                    }
#else
            for (uint n = 0; n < N; n++)
                    ...
                    for (uint i = 0; i < PER_WI_TILE_COL; i++) {
                        do {
                            ...
                            newVal         = as_uint(sum[n][j][i] + previous_value);
                                ...
                                atomic_cmpxchg(
                                        (__global
                                                 uint*)(pOut + j * STRIDE_IN + i + n * OUT_BATCH_STRIDE),
                                        prevVal,
                                        newVal)
                                != prevVal);
                    }
            ...
            if (IN_COL_START + OUT_WI_COL_START + 1 < STRIDE_IN) {
                for (uint n = 0; n < N; n++)
                    for (uint j = 0; j < PER_WI_TILE_ROW; j++)
                        for (uint i = 0; i < PER_WI_TILE_COL; i++) {
                            pOut[j * STRIDE_IN + i + n * OUT_BATCH_STRIDE] *=
                                    (pOut[j * STRIDE_IN + i + n * OUT_BATCH_STRIDE] > 0.0f ? 1.0f
                                                                                           : slope);
                        }
            } else if (IN_COL_START + OUT_WI_COL_START < STRIDE_IN) {
                for (uint n = 0; n < N; n++)
            ...
            if (lid_x == 0) {
                do {
                    newVal = BARRIER_MARK;
                                 ...
                                 (__global uint*)(pBarrier + COUNTER_INDEX), BARRIER_MARK, newVal)
                         != BARRIER_MARK);
            }
            barrier(CLK_GLOBAL_MEM_FENCE);
#endif
                ...
                if (lid_x == 0) {
                    do {
                        newVal = 0;
                                     ...
                                     (__global uint*)(pCounter + COUNTER_INDEX),
                                     GSU_MINUS_ONE,
                                     newVal)
                             != GSU_MINUS_ONE);
