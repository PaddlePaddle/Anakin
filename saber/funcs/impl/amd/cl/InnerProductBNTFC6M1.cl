    ...
    __global float *c)
{
	...
	int offset = ((grid_x << 6) + ((lid_x >> 6) * 12544) + (lid_x & 63)) % STRIDE;
	...
	for (int l = 0; l < 2; l++, offset = temp_offset)
	{
		...
		for (int i = 0; i < ITER; i++, offset = (offset + 64) % STRIDE)
		{
			...
			for (int k = 0; k < 4; k++)
			{
				sum += shared_a[(lid_x >> 6 << 6) + ((lid_x & 15) << 2) + k] * shared_b[(lid_x >> 6 << 2) + ((lid_x & 63) >> 4)][((lid_x & 15) << 2) + k];
			}
			barrier(CLK_LOCAL_MEM_FENCE);
		}
	...
	if (lid_x < 8)
	{
		c[(grid_x << 3) + lid_x] = result[lid_x >> 2][(lid_x & 3) << 4] + bias[(grid_x << 3) + lid_x];
	}
}
