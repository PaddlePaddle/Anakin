...
#include "jit_avx512_conv1x1_act_kernel.h"
...
SaberStatus jit_avx512_common_1x1_conv_kernel::init_conf(jit_1x1_conv_conf_t &jcp,
    ...
    const Shape &weights_d,
    int group,
    int dilation_h,
    int dilation_w,
    bool with_relu, float relu_negative_slope,
    int nthreads, bool with_bias, bool reduce_src) {
    jcp.ngroups = group; // with_groups ? weights_d.dims()[0] : 1;
    const bool with_groups = jcp.ngroups > 1; // weights_d.ndims() == src_d.ndims() + 1;
    jcp.mb = conv_d.src_d[0];

    jcp.oc = conv_d.dst_d[1] / jcp.ngroups;
    jcp.ic = conv_d.src_d[1] / jcp.ngroups;

    jcp.ih = conv_d.src_d[2];
    jcp.iw = conv_d.src_d[3];
    jcp.oh = conv_d.dst_d[2];
    jcp.ow = conv_d.dst_d[3];

    jcp.kh = weights_d[with_groups + 2];
    jcp.kw = weights_d[with_groups + 3];

    jcp.t_pad = conv_d.padding[0][0];
    jcp.l_pad = conv_d.padding[0][1];

    jcp.stride_h = conv_d.strides[0];
    jcp.stride_w = conv_d.strides[1];
    ...
    args_ok = true
        ...
        && jcp.kh == 1 && jcp.kw == 1;
    if (!args_ok) return SaberUnImplError;
    ...
    {
        //constexpr memory_format_t weights_formats[2] = { OIhw16i16o , gOIhw16i16o };
        //memory_format_t weights_format = weights_formats[with_groups];
