...
#ifndef ANAKIN_SABER_FUNCS_IMPL_X86_JIT_AVX2_CONV_H
#define ANAKIN_SABER_FUNCS_IMPL_X86_JIT_AVX2_CONV_H
...
class JitAvx2Conv : public ImplBase<
        X86, OpDtype, ConvEltwiseParam <X86>> {
public:
    typedef typename DataTrait<X86, OpDtype>::Dtype OpDataType;

    JitAvx2Conv() {kernel = nullptr;}
    ~JitAvx2Conv() {
        if (kernel) {
            delete kernel;
        }
...
private:
    ...
    std::unique_ptr<Tensor<X86>> weights_internal;
    std::unique_ptr<Tensor<X86>> bias_internal;
