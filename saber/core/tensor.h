...
public:
    ...
    const float *get_scale_data() const {
        return (const float*)_scale_buf.get_data();
    }
     ...
     *  \param offset
     */
    SaberStatus set_shape_without_layout(Shape valid_shape, Shape shape = Shape(), Shape offset = Shape()) {
