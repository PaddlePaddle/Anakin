...
SaberStatus Net::load_model_info(std::istream &stream) {
    ...
    auto str_to_dtype = [](std::string precision_name) -> DataType{
        if (precision_name == "fp32"){
            return AK_FLOAT;
        }else if (precision_name == "int8"){
            return AK_INT8;
        }else {
            LOGF("Now only support fp32 and int8");
        }
    };

    std::string strtmp;
    int model_version;
    const int lite_version = MAJOR * 100 + MINOR * 10 + REVISION;
    stream >> strtmp >> model_version;
    if (lite_version != model_version){
        LOGE("ERROR: model version: %d.%d.%d is not equal to lite version: %d.%d.%d\n", \
            model_version / 100, model_version % 100 / 10, model_version % 100 % 10, MAJOR, MINOR, REVISION);
    ...
    for (int i = 0; i < tensor_size; ++i) {
        ...
        int val_size;
        int real_size;
        std::vector<float> tensor_calibrator(1);
        std::string tensor_precision;
        std::string tensor_name;
        std::string shared_name;

        stream >> tensor_name >> tensor_precision >> tensor_calibrator[0] >> val_size;
//        tensor_calibrator[0] *= 2.f;
        ...
        if (_tensors.find(tensor_name) != _tensors.end()) {
            LOGE("ERROR: tensor is already registered: %s\n", tensor_name.c_str());
            return SaberInvalidValue;
        }
        Tensor<CPU>* tensor = new Tensor<CPU>;
        Shape vshape = val_sh;
        ...
        if (is_shared > 0) {
            ...
            if (it == _tensors.end()) {
                LOGE("ERROR: could not find tensor: %s\n", shared_name.c_str());
                return SaberInvalidValue;
            }
            tensor->set_dtype(str_to_dtype(tensor_precision));
            tensor->set_scale(tensor_calibrator);
//            LOGI("tensor name: %s, tensor_precision: %s, tensor_scale: %f\n",\
//                    tensor_name.c_str(), tensor_precision.c_str(), tensor->get_scale()[0]);
            tensor->share_from(*it->second);
        } else {
            tensor->re_alloc(rshape/*, str_to_dtype(tensor_precision)*/);
            tensor->set_dtype(str_to_dtype(tensor_precision));
            tensor->set_scale(tensor_calibrator);
//            LOGI("tensor name: %s, tensor_precision: %s, tensor_scale: %f\n",\
//                    tensor_name.c_str(), tensor_precision.c_str(), tensor->get_scale()[0]);
...
//        LOGI("%s vshape: %d,%d,%d,%d, rshape: %d,%d,%d,%d share:%s\n",
//               tensor_name.c_str(),
//               val_sh[0],
//               val_sh[1],
//               val_sh[2],
...
//    LOGI("ops number: %d\n", ops_num);
    ...
    for (int i = 0; i < ops_num; ++i) {
        std::string op_type;
        std::string op_name;
        std::string op_precision;
        int in_num = 0;
        int out_num = 0;
        stream >> op_type >> op_name >> op_precision >> in_num >> out_num;
//        LOGI("op type: %s, op_name: %s, precision: %s, ins: %d, outs: %d\n", op_type.c_str(), op_name.c_str(), op_precision.c_str(), in_num, out_num);
        std::vector<Tensor<CPU>*> tensor_ins;
        for (int j = 0; j < in_num; ++j) {
            std::string in_tensor_name;
            stream >> in_tensor_name;  
            auto it = _tensors.find(in_tensor_name);
            if (it == _tensors.end()) {
                LOGE("ERROR: tensor name: %s not exits\n", in_tensor_name.c_str());
                return SaberInvalidValue;
            }
//            LOGI("find ins: %s\n", in_tensor_name.c_str());
        ...
        std::vector<Tensor<CPU>*> tensor_outs;
...
//#endif
//        LOGI("op name : %s, op precision: %s\n", op_name.c_str(), op_precision.c_str());
        op->load_param(stream, _weights);
        if (op->set_op_precision(str_to_dtype(op_precision)) != SaberSuccess){
            op->set_op_precision(AK_FLOAT);
            LOGE("ERROR: Op: %s not support precision %s, use default precision fp32\n", \
                    op_name.c_str(), op_precision.c_str());
        }
        _ops[i] = op;
    }
...
SaberStatus Net::load_model(const char *info_path, const char *weights_path) {
    ...
    if (flag != SaberSuccess) {
        LOGE("ERROR: load weights faild: %s\n", weights_path);
        return SaberInvalidValue;
    }
    fp_w.close();
...
SaberStatus Net::load_model(const void *info_memory, size_t info_size,
    ...
    if (flag != SaberSuccess) {
        LOGE("ERROR: load model weights faild\n");
        return SaberInvalidValue;
    }
    ...
    if (flag != SaberSuccess) {
        LOGE("ERROR: load model info faild\n");
        return SaberInvalidValue;
    }
    //! get default input shape
    _last_input_shapes.resize(_ins.size());
...
SaberStatus Net::load_model(const void *merged_memory, size_t mem_size) {
    ...
    if (flag != SaberSuccess) {
        LOGE("ERROR: load merged model weights faild\n");
        return SaberInvalidValue;
    }
    //! load model
    flag = load_model_info(stream);
    if (flag != SaberSuccess) {
        LOGE("ERROR: load merged model info faild\n");
        return SaberInvalidValue;
    }
...
SaberStatus Net::prediction() {
    if (_weights == nullptr) {
        LOGE("ERROR: load model before prediction\n");
    ...
    for (int i = 0; i < _ops.size(); ++i) {
        if (_ops[i]->dispatch(_tensor_ins[i], _tensor_outs[i]) != SaberSuccess) {
            LOGE("ERROR: %s dispatch failed\n", _ops[i]->get_op_name());
            if (1){//std::string(_ops[i]->get_op_name()) == show_name) {
                for (int j = 0; j < _tensor_outs[i].size(); ++j) {
                    double meanval = tensor_mean(*_tensor_outs[i][j]);
                    LOGE("op: %s, mean: %.6f, scale: %.6f, ac: %.6f\n", _ops[i]->get_op_name(), meanval, _tensor_outs[i][j]->get_scale()[0], _tensor_outs[i][j]->get_scale()[0] * meanval);
                }
                //print_tensor(*_tensor_outs[i][0]);
            }
            break;
        }
#ifdef ENABLE_DEBUG
        //std::string show_name = "asymconv2_3_2_y";
        if (1){//std::string(_ops[i]->get_op_name()) == show_name) {
            for (int j = 0; j < _tensor_outs[i].size(); ++j) {
                double meanval = tensor_mean(*_tensor_outs[i][j]);
                LOGE("op: %s, mean: %.6f, scale: %.6f, ac: %.6f\n", _ops[i]->get_op_name(), meanval, _tensor_outs[i][j]->get_scale()[0], _tensor_outs[i][j]->get_scale()[0] * meanval);
...
SaberStatus Net::init() {
    for (int i = 0; i < _ops.size(); ++i) {
        if (_ops[i]->compute_output_shape(_tensor_ins[i], _tensor_outs[i]) != SaberSuccess) {
            LOGE("ERROR: %s compute shape failed\n", _ops[i]->get_op_name());
            return SaberNotInitialized;
        }
        if (_ops[i]->init(_tensor_ins[i], _tensor_outs[i], *_ctx) != SaberSuccess) {
            LOGE("ERROR: %s init failed\n", _ops[i]->get_op_name());
            return SaberNotInitialized;
        }
    }
    return SaberSuccess;
...
std::vector<Tensor<CPU> *> Net::get_input() {
    std::vector<Tensor<CPU>*> ins;
    for (int i = 0; i < _ins.size(); ++i) {
...
Tensor<CPU> * Net::get_input(std::string name) {
    if (_tensors.find(name) == _tensors.end()) {
        LCHECK_EQ(true, false, "ERROR: input is not exits\n");
        return nullptr;
    }
    return _tensors[name];
}

std::vector<Tensor<CPU>*> Net::get_output() {
    std::vector<Tensor<CPU>*> out;
    for (int i = 0; i < _outs.size(); ++i) {
...
Tensor<CPU> * Net::get_output(std::string name) {
    if (_tensors.find(name) == _tensors.end()) {
        LCHECK_EQ(true, false, "ERROR: output tensor is not exits\n");
        return nullptr;
    }
    return _tensors[name];
}

void Net::set_device_arch(ARMArch arch) {
    _ctx->set_arch(arch);
}
