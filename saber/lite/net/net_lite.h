...
public:
    ...
    /**
     * \brief set cpu arch, get from system info in the soon feature
     * @param arch: input arch
     */
    void set_device_arch(ARMArch arch);
     ...
     * @return
     */
    std::vector<Tensor<CPU>*> get_input();
     ...
     */
    Tensor<CPU>* get_input(std::string name);

    /**
     * \brief get output tensor
     * @return
     */
    std::vector<Tensor<CPU>*> get_output();
     ...
     */
    Tensor<CPU>* get_output(std::string name);
...
private:
    ...
    std::vector<std::vector<Tensor<CPU>*>> _tensor_ins;
    //! container which holds the output tensors of each op
    std::vector<std::vector<Tensor<CPU>*>> _tensor_outs;
    //! container which holds all tensors
    std::map<std::string, Tensor<CPU>*> _tensors;
    //! op instances
