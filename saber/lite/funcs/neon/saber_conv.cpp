...
template <>
SaberStatus SaberConv2D<float>::create(\
    const std::vector<Tensor<float> *>& inputs, \
    std::vector<Tensor<float> *>& outputs,\
    ConvParam<Tensor<float>> &conv_param, Context &ctx) {
    ...
    _bias_term = conv_param.bias()->valid_size() > 0;
...
template <>
SaberStatus SaberConv2D<float>::init(\
    const std::vector<Tensor<float> *>& inputs, \
    std::vector<Tensor<float> *>& outputs, \
    ConvParam<Tensor<float>> &conv_param, Context &ctx) {
...
template <>
SaberStatus SaberConv2D<float>::dispatch(\
    const std::vector<Tensor<float> *>& inputs, \
    std::vector<Tensor<float> *>& outputs, \
    ConvParam<Tensor<float>> &conv_param) {
    ...
    _impl(*inputs[0], *outputs[0], weight, bias, conv_param.group, _kw, _kh, \
            ...
            _bias_term, _flag_relu, _gemmer, (void*)_workspace_data->mutable_data());
    return SaberSuccess;
}
