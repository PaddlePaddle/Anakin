...
void pooling2x2s2_max(const void* din, void* dout, \
    ...
    if (global) {
        //printf("not supported in this funcs, instead, use the basic func\n");
        LOGE("not supported in this funcs, instead, use the basic func\n");
        return;
    }
#if 0
    int cnt_col = (wout + 3) / 4;
    int col_remain = cnt_col * 4 - hin;
    int unrool_loop = cnt_col / 2 - 1;
    int loop_remain = cnt_col - unrool_loop * 2;
...
#pragma omp parallel for
        for (int c = 0; c < chout; ++c) {
            const float* ptr_in_c = din + (n * chin + c) * size_in_channel;
            float* ptr_out_c = dout + (n * chout + c) * size_out_channel;

            for (int h = 0; h < hout - 1; ++h) {

                const float* ptr0 = ptr_in_c + h * 2 * win;
                const float* ptr1 = ptr0 + win;
                float* ptr_out = ptr_out_c + h * wout;

                int loop = unrool_loop;
#ifdef __aarch64__
#else
                asm volatile(
                "vld2.32  {d0-d3}, [%[ptr0]]!                           @ load r0, 8\n"
                        "vld2.32  {d4-d7}, [%[ptr1]]!                   @ load r1, 8\n"
                        "vld2.32  {d8-d11}, [%[ptr0]]!                  @ load r0, 8\n"
                        "vld2.32  {d12-d15}, [%[ptr1]]!                 @ load r1, 8\n"
                        "1:                                             @ main loop\n"
                        "vmax.f32  q8, q0, q1                           @ max of r0\n"
                        "vld2.32  {d0-d3},  [%[ptr0]]!                  @ load r0, 8\n"
                        "vmax.f32  q9, q2, q3                           @ max of r1"
                        "vld2.32  {d4-d7},  [%[ptr1]]!                  @ load r1, 8\n"
                        "vmax.f32  q10, q4, q5                          @ max of r0\n"
                        "vld2.32  {d8-d11}, [%[ptr0]]!                  @ load r0, 8\n"
                        "vmax.f32  q11, q6, q7                          @ max of r1\n"
                        "vld2.32  {d12-d15},[%[ptr1]]!                  @ load r1, 8\n"
                        "vmax.f32  q8, q8, q9                           @ max of r0, r1\n"
                        "vmax.f32  q10, q10, q11                        @ max of r0, r1\n"
                        "subs       %[loop], #1                         @ loop --\n"
                        "vst1.32   {d16-d17}, [%[ptr_out]]!             @ write first 4 result\n"
                        "vst1.32   {d18-d21}, [%[ptr_out]]!             @ write second 4 result\n"
                        "bne    1b                                      @ jump  to main loop\n"
                : [ptr0] "+r" (ptr0), [ptr1] "+r" (ptr1), [ptr_out] "+r" (ptr_out), [loop] "+r" (loop)
                :
...
#pragma omp parallel for
        for (int c = 0; c < chout; c++) {
            ...
            int cnt_num = win / 8;
            //w = w_in - 8;
            int cnt_num1 = (win - cnt_num * 8 - 1) / 2;
            float* dr_out = data_out_channel;
            const float* dr0 = r0;
            const float* dr1 = r1;
            const float* dr2 = r2;
...
//    LOG(INFO) << "pad_right: "<<pad_right<<", w_unroll_size: "<<w_unroll_size<<", w_unroll_remian" << w_unroll_remian;
...
int16x8_t addw_256(const signed char* in){
...
int16x8_t addw_n(const signed char* in, int n){
    int16x8_t sum = vdupq_n_s16(0);
#ifdef __aarch64__   
    for (int i = 0; i < n; ++i){
        int8x8_t data_in = vld1_s8(in);
...
void pooling_global_int8(const void* din, void* dout, \
    ...
    for (int n = 0; n < num; ++n) {
        signed char* data_out_batch = data_out + n * chout;
        const signed char* data_in_batch = data_in + n * chin * size_channel_in;
        if (type == Pooling_max) {
#pragma omp parallel for
            for (int c = 0; c < chout; ++c) {
                const signed char* data_in_channel = data_in_batch + c * size_channel_in;
                int i = 0;
                signed char minval = std::numeric_limits<signed char>::min();
                signed char max_tmp = minval;
                signed char max0 = minval;
                signed char max1 = minval;
...
#pragma omp parallel for
        for (int c = 0; c < chout; c++) {
            signed char* data_out_channel = data_out_batch + c * size_channel_out;
            const signed char* data_in_channel = data_in_batch + c * size_channel_in;
            const signed char* r0 = data_in_channel;
            const signed char* r1 = r0 + win;
            int h = 0;
            for (; h < h_even; h += 2) {
                int w = 0;
#ifdef  __aarch64__
                for (; w < w_unroll_size_32; w += 32) {
                    ...
                    vst1q_s8(&data_out_channel[w >> 1], dmax);
                ...
                for (; w < w_unroll_size_32 + w_unroll_size_16; w += 16) {
                    ...
                    vst1_s8(&data_out_channel[w >> 1], dmax);
                }
#else
                int cnt_32 = w_unroll_size_32 >> 5;
                int cnt_16 = w_unroll_size_16 >> 4;
                const signed char* dr0 = r0;
                const signed char* dr1 = r1;
                signed char* dr_out_32 = data_out_channel;
                signed char* dr_out_16 = dr_out_32 + (cnt_32 << 4);
                if (cnt_32 > 0){
                    asm volatile(
                            "1:                                              @main loop\n"
            ...
            for (; h < hin; h++) { 
                ...
                if (cnt_16 > 0){
                    ...
                    asm volatile(
                            ...
                            "vst1.s8   d2, [%[dr_out_16]]!                   @vst1 d2, dr_out\n"
                            "subs      %[cnt_16], #1                         @subs cnt, 1\n"
                            "bne       1b                                    @bne cnt\n"
                    :[dr0] "+r" (dr0), [dr_out_16] "+r" (dr_out_16), [cnt_16] "+r" (cnt_16)
                    :"r" (dr0), "r" (dr_out_16), "r"(cnt_16)
                    :"q0", "q1"
                    );
                }
#endif  //__aarch64__
                for (; w < w_even; w += 2) {
                    data_out_channel[w >> 1] = std::max(r0[w], r0[w + 1]);
                }
                for (; w < win; ++w) { // run 0 or 1 time
                    data_out_channel[w >> 1] = r0[w];
                }
            }
        }
        ...
        for (int c = 0; c < chout; c++) {
            ...
            for (; h < h_even; h += 2) {
                ...
                for (;w < w_unroll_size_16; w += 16) {
                    ...
                    vst1_s8(&data_out_channel[w >> 1], dmax.val[0]);
                }
#else
                int cnt_16 = w_unroll_size_16 >> 4;
                const signed char* dr0 = r0;
                const signed char* dr1 = r1;
                signed char* dr_out_16 = data_out_channel;
                if (cnt_16 > 0){
                    asm volatile(
                            "1: \n"
                            "vld1.s8   {d0-d1}, [%[dr0]]!  \n"
                ...
                for (; w < w_even; w += 2) {
                    ...
                    signed char ave = char(sum / 4);
                    data_out_channel[w >> 1] = ave;
                }
                for (; w < win; ++w) { // run 0 or 1 time
                    int sum = int(r0[w]) + int(r1[w]);
                    signed char ave = char(sum / 2);
                    data_out_channel[w >> 1] = ave;
                }
                r0 += w_in_2;
                r1 += w_in_2;
...
void pooling3x3s1p1_max_int8(const void* din, void* dout, \
    ...
    for (int n = 0; n < num; ++n) {
        signed char* data_out_batch = data_out + n * ch_out * size_channel_out;
        const signed char* data_in_batch = data_in + n * ch_in * size_channel_in;
#pragma omp parallel for
        for (int c = 0; c < ch_out; c++) {
            signed char* data_out_channel = data_out_batch + c * size_channel_out;
            const signed char* data_in_channel = data_in_batch + c * size_channel_in;
            const signed char* r0 = data_in_channel;
            const signed char* r1 = r0 + w_in;
            const signed char* r2 = r1 + w_in;
            signed char* dr_out = data_out_channel;
            const signed char* dr0 = r0;
            const signed char* dr1 = r1;
            const signed char* dr2 = r2;
            int w = 0;
            int cnt = 1;
            data_out_channel[0] = std::max(std::max(r0[0], r0[1]), std::max(r1[0], r1[1]));
#ifdef __aarch64__
            ...
            for (int i = 0; i < cnt_14; ++i){
                ...
                vst2_s8(&data_out_channel[cnt], vmax); 
                cnt += 14;
                w += 14;
            }
            for (int i = 0; i < cnt_6; ++i){
                ...
                vst1_s8(&data_out_channel[cnt], vmax.val[0]); 
                cnt += 6;
                w += 6;
            }
            ...
            if (cnt_14_loop > 0){
                asm volatile(
                        ...
                        "vst2.s8  {d4-d5}, [%[dr_out]]!                 @vst2 d4-d5, drout\n"
            ...
            if (cnt_6_loop > 0){
                asm volatile(
                        ...
                        "vst1.s8  d8, [%[dr_out]]!                      @vst d8, drout\n"
                        "subs     %[dr_out], #2                         @sub drout, 2\n"
                        "subs     %[dr0], #2                            @sub dr0, 2\n"
                        "subs     %[dr1], #2                            @sub dr1, 2\n"
            ...
            for (int j = 0; j < cnt_6_remain; j++){
                signed char tmp_max = std::max(r0[j + w], r1[j + w]);
                tmp_max = std::max(tmp_max, std::max(r0[j + w + 1], r1[j + w + 1]));
                tmp_max = std::max(tmp_max, std::max(r0[j + w + 2], r1[j + w + 2]));
                data_out_channel[j + w + 1] = tmp_max;
            }
            signed char tmp = std::max(r0[w_in - 2], r1[w_in - 2]);
            tmp = std::max(tmp, std::max(r0[w_in - 1], r1[w_in - 1]));
            data_out_channel[w_out - 1]  = tmp;
            data_out_channel += w_out;
            int h = 0;
            for (; h < h_in - 2; h += 1) {
                signed char maxr0 = std::max(r0[0], r0[1]);
                signed char maxr1 = std::max(r1[0], r1[1]);
                signed char maxr2 = std::max(r2[0], r2[1]);
                data_out_channel[0] = std::max(std::max(maxr0, maxr1), maxr2);
                w = 0;
                cnt = 1;
#ifdef __aarch64__
...
void pooling3x3s2p1_max_int8(const void* din, void* dout, \
    ...
    if (win < 3 || hin < 3){
        pooling_basic_int8(din, dout, num, chout, hout, wout, chin, hin, win, \
                            type, global, kernel_w, kernel_h, stride_w, stride_h, \
                            pad_w, pad_h);
        return;
    }
    int ch_in = chin;
...
#pragma omp parallel for
        for (int c = 0; c < chout; c++) {
            ...
            for (int i = 0; i < cnt_7; ++i) {
                ...
                vst1_s8(&data_out_channel[cnt], vmax);
                cnt += 7;
                w += 14;
            }

#else
            const signed char* dr0 = r0 + 1;
            const signed char* dr1 = r1 + 1;
            const signed char* dr2 = r2 + 1;
            signed char* dr_out = data_out_channel + 1;
            ...
            for (;h < h_out - 2; ++h){
                ...
                signed char maxr1 = std::max(r1[0], r1[1]);
                signed char maxr2 = std::max(r2[0], r2[1]);
                data_out_channel[0] = std::max(maxr0, std::max(maxr1, maxr2));
#ifdef __aarch64__
                for (int i = 0; i < cnt_7; ++i) {
                    int8x16_t vr0 = vld1q_s8(&r0[w]);
                    int8x16_t vr1 = vld1q_s8(&r1[w]);
                ...
                for (int j = 0; j < cnt_7_remain; ++j){
                    ...
                    data_out_channel[(w >> 1) + 1] = std::max(tmp_max, r2[w + 2]);
                    w += 2;
                }
                //last
                int ld_cnt = w_in - 2 * w_out + 3;
            ...
            if (h_in & 1){
                ...
                if (cnt_7_loop > 0){
                    asm volatile(
                            ...
                            "subs     %[dr0], #2                          @sub dr0, 2\n"
                            "subs     %[dr1], #2                          @sub dr1, 2\n"
                            "vst1.s8  d10, [%[dr_out]]!                   @vst1 d10, dr_out\n"
            ...
            if (h_in & 1){
                ...
                for (int j = 0; j < cnt_7_remain; ++j){
                    signed char tmp_max = std::max(r0[w], r0[w + 1]);
                    data_out_channel[(w >> 1) + 1] = std::max(tmp_max, r0[w + 2]);
                    w += 2;
                }
            }
            ...
            switch (ld_case){
                case 12:
                    data_out_channel[w_out - 1] = std::max(r0[w], r0[w + 1]);
                    break;
                case 22:
                    tmp_max = std::max(r0[w], r0[w + 1]);
                    data_out_channel[w_out - 1] = std::max(tmp_max, std::max(r1[w], r1[w + 1]));
                    break;
                case 11:
                    data_out_channel[w_out - 1] = r0[w];
                    break;
                case 21:
                    data_out_channel[w_out - 1] = std::max(r0[w], r1[w]);
                    break;
                default:
                    break;
            }
        ...
        for (int c = 0; c < chout; c++) {
            ...
            for (;h < h_out - 1; ++h){
                ...
                for (int i = 0; i < cnt_7; ++i) {
                    ...
                    int8x8_t vmax1p = vpmax_s8(vget_low_s8(vmax1), vget_high_s8(vmax1));
                    int8x8_t vmax = vmax_s8(vmax0p, vmax1p);
                    vst1_s8(&data_out_channel[w >> 1], vmax);
                    w += 14;
                }
#else           
                dr0 = r0;
                ...
                for (int j = 0; j < cnt_7_remain; ++j){
                    ...
                    data_out_channel[(w >> 1)] = std::max(tmp_max, r2[w + 2]);
                    w += 2;
                }
                //last
                signed char tmp_max = std::numeric_limits<signed char>::min();
            ...
            if (h_in & 1){
                ...
                for (int j = 0; j < cnt_7_remain; ++j){
                    ...
                    tmp_max = std::max(tmp_max, std::max(r0[w + 1], r1[w + 1]));
                    tmp_max = std::max(tmp_max, std::max(r0[w + 2], r1[w + 2]));
                    data_out_channel[(w >> 1)] = tmp_max;
                    w += 2;
                }
            }
            int last_ld_cnt = h_in - 2 * h_out + 2;
            int ld_case = last_ld_cnt * 10 + ld_cnt;
            signed char tmp_max = 0;
            //last out load case
            switch (ld_case){
                case 22:
                    tmp_max = std::max(r0[w], r0[w + 1]);
                ...
                case 23:
                    ...
                    tmp_max = std::max(tmp_max, std::max(r1[w], r1[w + 1]));
                    data_out_channel[w_out - 1] = std::max(tmp_max, r1[w + 2]);
                    break;
                case 32:
                    tmp_max = std::max(r0[w], r0[w + 1]);
                    tmp_max = std::max(tmp_max, std::max(r1[w], r1[w + 1]));
