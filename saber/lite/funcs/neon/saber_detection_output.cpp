...
SaberDetectionOutput::SaberDetectionOutput(ParamBase *param) {
    _param = (DetectionOutputParam*)param;
...
SaberStatus SaberDetectionOutput::load_param(ParamBase *param) {
    if (this->_flag_create_param) {
        delete _param;
        _param = nullptr;
...
SaberStatus SaberDetectionOutput::set_op_precision(DataType ptype) {
...
SaberStatus SaberDetectionOutput::compute_output_shape(const std::vector<Tensor<CPU> *> &inputs,
                                                       std::vector<Tensor<CPU> *> &outputs) {
    if (!this->_flag_param) {
        LOGE("ERROR: load detection_output param first\n");
...
SaberStatus SaberDetectionOutput::init(const std::vector<Tensor<CPU>*>& inputs, \
    ...
    LCHECK_EQ(_num_priors * _num_loc_classes * 4, inputs[0]->valid_size() / num, \
        "ERROR: Number of priors: %d must match number of location predictions: %d.\n", \
        _num_priors * _num_loc_classes * 4, inputs[0]->valid_size() / num);
    LCHECK_EQ(_num_priors * _class_num, inputs[1]->valid_size() / num, \
        "ERROR: Number of priors: %d must match number of confidence predictions: %d.\n", \
        _num_priors * _class_num, inputs[1]->valid_size() / num);
...
SaberStatus SaberDetectionOutput::dispatch(
    ...
    if (!this->_flag_init) {
        LOGE("ERROR: init detection_output first\n");
        return SaberNotInitialized;
    }
    ...
    Tensor<CPU>* t_loc = inputs[0];
    Tensor<CPU>* t_conf = inputs[1];
    Tensor<CPU>* t_prior = inputs[2];
    ...
    // fixme
    ops.ops = 0;
    LOGI("detection_output %s: time: %f\n", this->_op_name.c_str(), ts);
    OpTimer::add_timer("detection_optput", ops);
