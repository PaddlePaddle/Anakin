...
SaberEltwiseAct::SaberEltwiseAct(ParamBase *param) {
    _param = (EltwiseActParam*)param;
...
SaberStatus SaberEltwiseAct::load_param(ParamBase *param) {
    if (this->_flag_create_param) {
        delete _param;
        _param = nullptr;
...
SaberStatus SaberEltwiseAct::set_op_precision(DataType ptype) {
...
SaberStatus SaberEltwiseAct::compute_output_shape(const std::vector<Tensor<CPU> *> &inputs,
    ...
    for (int i = 1; i < inputs.size(); ++i) {
        LCHECK_EQ(inputs[0]->num(), inputs[i]->num(), "ERROR: input size must be the same\n");
        LCHECK_EQ(inputs[0]->channel(), inputs[i]->channel(), "ERROR: input size must be the same\n");
        LCHECK_EQ(inputs[0]->height(), inputs[i]->height(), "ERROR: input size must be the same\n");
        LCHECK_EQ(inputs[0]->width(), inputs[i]->width(), "ERROR: input size must be the same\n");
    }
...
SaberStatus SaberEltwiseAct::dispatch(\
    ...
    const float* din_a = static_cast<const float*>(inputs[0]->data());
    const float* din_b = static_cast<const float*>(inputs[1]->data());
    ...
    if (_param->_has_activation && _param->_activation_param._act_type == 2){//relu
        ...
        for (int i = 2; i < inputs.size(); ++i) {
            din_a = static_cast<const float*>(inputs[i]->data());
    ...
    if (_param->_has_activation && _param->_activation_param._act_type == 10){//relu
        _impl(din_a, din_b, dout, num, channel, channel_size, _param->_eltwise_param._coef,  \
             _param->_activation_param._prelu_channel_shared, \
             static_cast<const float*>(_param->_activation_param._prelu_weights.data()));
        for (int i = 2; i < inputs.size(); ++i) {
            din_a = static_cast<const float*>(inputs[i]->data());
