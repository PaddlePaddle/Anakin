...
SaberSoftmax::SaberSoftmax(ParamBase *param) {
    if (this->_flag_create_param) {
        delete _param;
        _param = nullptr;
        this->_flag_create_param = false;
    }
    _param = (SoftmaxParam*)param;
...
SaberStatus SaberSoftmax::load_param(ParamBase *param) {
    _param = (SoftmaxParam*)param;
    this->_flag_param = true;
    return SaberSuccess;
}

SaberStatus SaberSoftmax::set_op_precision(DataType ptype) {
...
#endif
SaberStatus SaberSoftmax::compute_output_shape(const std::vector<Tensor<CPU> *> &inputs,
                                               std::vector<Tensor<CPU> *> &outputs) {
    if (!this->_flag_param) {
        LOGE("ERROR: load softmax param first\n");
        return SaberNotInitialized;
    }
    return outputs[0]->set_shape(inputs[0]->valid_shape());
}

SaberStatus SaberSoftmax::init(const std::vector<Tensor<CPU> *> &inputs,
...
SaberStatus SaberSoftmax::dispatch(const std::vector<Tensor<CPU>*>& inputs, \
    std::vector<Tensor<CPU>*>& outputs) {

    if (!this->_flag_init) {
        LOGE("ERROR: init softmax first\n");
        return SaberNotInitialized;
    }
