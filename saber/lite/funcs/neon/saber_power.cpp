...
SaberPower::SaberPower(ParamBase *param) {
    _param = (PowerParam*)param;
    this->_flag_param = true;
}

SaberStatus SaberPower::load_param(ParamBase *param) {
...
SaberStatus SaberPower::set_op_precision(DataType ptype) {
...
SaberStatus SaberPower::compute_output_shape(const std::vector<Tensor<CPU> *> &inputs,
                                               std::vector<Tensor<CPU> *> &outputs) {

    if (!this->_flag_param) {
        LOGE("ERROR: load power param first\n");
...
//template <>
SaberStatus SaberPower::init(const std::vector<Tensor<CPU> *> &inputs,
...
//template <>
SaberStatus SaberPower::dispatch(const std::vector<Tensor<CPU> *> &inputs,
                                   std::vector<Tensor<CPU> *> &outputs) {

    if (!this->_flag_init) {
        LOGE("ERROR: init power first\n");
        return SaberNotInitialized;
    }
