...
SaberPermute::SaberPermute(ParamBase *param) {
    _param = (PermuteParam*)param;
...
SaberStatus SaberPermute::load_param(ParamBase *param) {
    if (this->_flag_create_param) {
        delete _param;
        _param = nullptr;
...
SaberStatus SaberPermute::set_op_precision(DataType ptype) {
...
SaberStatus SaberPermute::compute_output_shape(const std::vector<Tensor<CPU> *> &inputs,
                                               std::vector<Tensor<CPU> *> &outputs) {
    if (!this->_flag_param) {
        LOGE("ERROR: load permute param first\n");
    ...
    for (int i = 0; i < inputs.size(); ++i) {
        ...
        LCHECK_EQ(inputs[i]->valid_shape().size(), _param->_order.size(), "ERROR: permute order param is not valid\n");
...
//template <typename Dtype>
SaberStatus SaberPermute::init(const std::vector<Tensor<CPU> *> &inputs,
    ...
    LCHECK_EQ(inputs[0]->dims(), _param->_order.size(), "ERROR: permute order size is not match to input dims\n");
    // set _need_permute
    _need_permute = false;
...
SaberStatus SaberPermute::dispatch(\
    ...
    LOGI("permute %s: time: %f\n", this->_op_name.c_str(), ts);
    GOPS ops;
    // fixme
    ops.ops = 0;
    ops.ts = ts;
    OpTimer::add_timer("permute", ops);
