...
*/
//yuv bbbbggggrrr store bgrbgr dsth * dstw = (2 * srch) * srcw y_w = srcw, y_h = 2/3 * srch uv_w = srcw uv_h = 1/3 * srch
// dst_h = 3 * y_h
void nv12_to_bgr(const unsigned char* src, unsigned char* dst, int srcw, int srch, int dstw, int dsth){

    LCHECK_EQ(srcw, dstw, "ERROR: sizes of two valid shapes must be the same\n");
    LCHECK_EQ(srch * 2, dsth, "ERROR: sizes of two valid shapes must be the same\n");
    int y_h = srch * 2 / 3;
    int vu_h = srch - y_h;
    ...
    for (int i = 0; i < y_h; i += 2){
        ...
        unsigned char* ptr_bgr1 = dst + (i * 3) * dstw;
        unsigned char* ptr_bgr2 = ptr_bgr1 + dstw;
        unsigned char* ptr_bgr3 = ptr_bgr2 + dstw;
        unsigned char* ptr_bgr4 = ptr_bgr3 + dstw;
        unsigned char* ptr_bgr5 = ptr_bgr4 + dstw;
        unsigned char* ptr_bgr6 = ptr_bgr5 + dstw;
        //2*16
#ifdef __aarch64__
        asm volatile(
