...
*/
//yvu bbbbggggrrr store bgrbgr dsth * dstw = (2 * srch) * srcw y_w = srcw, y_h = 2/3 * srch uv_w = srcw uv_h = 1/3 * srch
// dst_h = 3 * y_h
void nv21_to_bgr(const unsigned char* src, unsigned char* dst, int srcw, int srch, int dstw, int dsth){

    LCHECK_EQ(srcw, dstw, "sizes of two valid shapes must be the same");
    LCHECK_EQ(srch * 2, dsth, "sizes of two valid shapes must be the same");
    int y_h = srch * 2 / 3;
    int vu_h = srch - y_h;
