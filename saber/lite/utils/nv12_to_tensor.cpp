...
void nv12_to_tensor(const unsigned char* nv12, TensorHf& output, int width, int height, float* means, float* scales) {
    ...
    // float r_val = r_means * r_scales;
    // float g_val = g_means * g_scales;
    // float b_val = b_means * b_scales;
    ...
    for (int h = 0; h < tile_h; h++){
        const unsigned char* nv12_y0 = nv12 + h * 2 * width;
        const unsigned char* nv12_y1 = nv12_y0 + width;
        const unsigned char* nv12_uv = uv_start + h * width;
...
#pragma omp parallel for
        for (int w = 0; w < dim8; w++){
            uint8x8_t  y0 = vld1_u8(nv12_y0);
            uint8x8_t  v = vld1_u8(nv12_uv);//uvuvuvuv
            uint8x8_t  u = vld1_u8(nv12_uv);//uvuvuvuv
            uint8x8_t  y1 = vld1_u8(nv12_y1);

            uint8x8x2_t vu = vtrn_u8(v, u);//uuuuvvvv

            uint16x8_t y16_0 = vmovl_u8(y0);
            uint16x8_t v16 = vmovl_u8(vu.val[1]);
            uint16x8_t u16 = vmovl_u8(vu.val[0]);
