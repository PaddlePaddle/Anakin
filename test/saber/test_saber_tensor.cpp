...
void tensor_constructor() {
    ...
    typedef typename IF<std::is_same<target_D, __host_target>::value, then_type, else_type>::Type
    flag_type;
...
void tensor_deepcopy() {
    ...
    th3.copy_from(th1);
    print_tensor(th3);
    tensor_cmp_host((const dtype*)th1.data(), (const dtype*)th3.data(), th3.size(), max_ratio,
                    max_diff);
