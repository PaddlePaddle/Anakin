...
bool lstm_test(test_lstm_params &param) {
    ...
    Shape inputShape({total_seq_len, param.input_size, 1, 1},Layout_NCHW);
    if (param.skip_input) {
    ...
    // weight's layout:
    // [ Wih Wfh Wch Woh ]
    Shape weightShape({param.layer_size, 4 * param.layer_size, 1, 1},Layout_NCHW);
    if (!param.skip_input) {
        // weight's layout:
        // [ Wix Wfx Wcx Wox ]
        // [ Wih Wfh Wch Woh ]
    ...
    Shape biasShape({1, bias_num * param.layer_size, 1, 1},Layout_NCHW);
    Tensor4f saberBias(biasShape);
    fill_tensor_host_rand(saberBias);

    Shape hiddenShape({param.mb * 2, param.layer_size, 1, 1},Layout_NCHW);
    ...
    Shape outputShape({total_seq_len, param.layer_size, 1, 1},Layout_NCHW);
