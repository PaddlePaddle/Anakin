...
#include "test_saber_func_eltwise_act.h"
#include "tensor_op.h"
#include "saber_types.h"
#include <vector>

using namespace anakin::saber;
TEST(TestSaberFuncEltwiseActNV, test_func_sum_act) {
    ...
    EltwiseActive<NV, AK_FLOAT> eltwise_dev;

    EltwiseType elt_type = Eltwise_sum;

    EltwiseParam<TensorDf4> eltwise_param(elt_type);
    ActivationParam<TensorDf4> activation_param(Active_relu);

    EltwiseActiveParam<TensorDf4> param(eltwise_param, activation_param);
...
#if 0
TEST(TestSaberFuncEltwiseActNV, test_func_prod_act) {
    ...
    Context<NV> ctx_dev(0, 1, 1);
    EltwiseActive<NV, AK_FLOAT> eltwise_act_dev;

    LOG(INFO) << "eltwise_active compute output shape";
    eltwise_act_dev.compute_output_shape(input_dev_4d, output_dev_4d, param);
    ...
    LOG(INFO) << "eltwise_active initialization";
    eltwise_act_dev.init(input_dev_4d, output_dev_4d, param, RUNTIME, SABER_IMPL, ctx_dev);


    LOG(INFO) << "eltwise_active compute";
    eltwise_act_dev(input_dev_4d, output_dev_4d, param, ctx_dev);
...
TEST(TestSaberFuncEltwiseActNV, test_func_max_act) {
    ...
    for (int i = 0; i < thin0.size(); ++i) {
        thin0.mutable_data()[i] = i - 4;
    }

    for (int i = 0; i < thin1.size(); ++i) {
        thin1.mutable_data()[i] = i - 4;
    }

    for (int i = 0; i < thin2.size(); ++i) {
        thin2.mutable_data()[i] = i - 3;
    }
 ...
 *                      75  76  77  78  79
 */
TEST(TestSaberFuncEltwiseActNV, test_func_prod_act_roi) {
...
TEST(TestSaberFuncEltwiseActNV, test_func_sum_act_roi) {
    ...
    EltwiseParam<TensorDf4> eltwise_param(elt_type, &tdcoeff);
    ActivationParam<TensorDf4> activation_param(Active_relu);
    EltwiseActiveParam<TensorDf4> param(eltwise_param, activation_param);
...
TEST(TestSaberFuncEltwiseActNV, test_func_max_roi) {
