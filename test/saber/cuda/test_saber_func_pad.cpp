...
void test_pad(std::vector<TensorType*>& inputs_big, std::vector<TensorType*>& outputs_big,
              ...
              PadParam<Tensor<NV, AK_FLOAT, NCHW>>& param, bool get_time, Context<NV>& ctx) {
    typedef Tensor<NV, AK_FLOAT, NCHW> TensorDf4;
    /*prepare data*/
    Pad<NV, AK_FLOAT, AK_FLOAT, AK_FLOAT, NCHW> pad;
    pad.compute_output_shape(inputs, outputs, param);
    ...
    pad.init(inputs, outputs, param, SPECIFY, SABER_IMPL, ctx);

    /*warm up*/
    pad(inputs, outputs, param, ctx);
...
TEST(TestSaberFuncNV, test_func_constructor) {
    ...
    std::vector<int> pad_c = {2, 2};
    std::vector<int> pad_h = {1, 1};
    std::vector<int> pad_w = {1, 1};
    PadParam<TensorDf4> param(/* axis_in*/ pad_c, /*offset*/pad_h, /*shape_in*/pad_w);
                ...
                for (auto output_share_sub : {
                    ...
                    for (auto get_time : {
                        ...
                        LOG(INFO) << input_share << "," << output_share << "," << input_share_sub << "," << output_share_sub
                                  << "," << get_time;
                        test_pad<TensorDf4>(inputs_big, outputs_big,
                                            inputs, outputs,
