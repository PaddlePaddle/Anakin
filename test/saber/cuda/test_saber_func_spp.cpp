...
void test_spp(std::vector<TensorType*>& inputs_big, std::vector<TensorType*>& outputs_big,
              ...
              SPPParam<Tensor<NV, AK_FLOAT, NCHW> >& param, bool get_time, Context<NV>& ctx) {
    typedef Tensor<NV, AK_FLOAT, NCHW> TensorDf4;
    /*prepare data*/
    Spp<NV, AK_FLOAT> spp;
    spp.compute_output_shape(inputs, outputs, param);
    ...
    spp.init(inputs, outputs, param, SPECIFY, SABER_IMPL, ctx);

    /*warm up*/
    spp(inputs, outputs, param, ctx);
...
TEST(TestSaberFuncNV, test_func_constructor) {
    ...
    SPPParam<TensorDf4> param(4, Pooling_max);
                ...
                for (auto output_share_sub : {
                    ...
                    for (auto get_time : {
                        ...
                        LOG(INFO) << input_share << "," << output_share << "," << input_share_sub << "," << output_share_sub
                                  << "," << get_time;
                        test_spp<TensorDf4>(inputs_big, outputs_big,
                                            inputs, outputs,
