...
void test_lrn(std::vector<TensorType*>& inputs_big, std::vector<TensorType*>& outputs_big,
              ...
              LrnParam<Tensor<NV, AK_FLOAT, NCHW>>& param, bool get_time, Context<NV>& ctx) {
    typedef Tensor<NV, AK_FLOAT, NCHW> TensorDf4;
    /*prepare data*/
    Lrn<NV, AK_FLOAT, AK_FLOAT, AK_FLOAT, NCHW, NCHW, NCHW> lrn;
    lrn.compute_output_shape(inputs, outputs, param);
    ...
    lrn.init(inputs, outputs, param, SPECIFY, SABER_IMPL, ctx);

    /*warm up*/
    lrn(inputs, outputs, param, ctx);
...
TEST(TestSaberFuncNV, test_func_constructor) {
    ...
    LrnParam<TensorDf4> param(/*local_size_in*/ 5, /*alpha_in*/1.f,  0.75,
                                           1.f, /*norm_region_in*/ ACROSS_CHANNELS);
                ...
                for (auto output_share_sub : {
                    ...
                    for (auto get_time : {
                        ...
                        LOG(INFO) << input_share << "," << output_share << "," << input_share_sub << "," << output_share_sub
                                  << "," << get_time;
                        test_lrn<TensorDf4>(inputs_big, outputs_big,
                                            inputs, outputs,
