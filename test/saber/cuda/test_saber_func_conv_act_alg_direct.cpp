...
TEST(TestSaberFuncNV, test_conv_fp32_conv_act_direct) {
    ...
    for (const auto& problem : problem_set) {
        ...
        ConvAct<NV, AK_FLOAT> saber_conv;
        ...
        Conv<NV, AK_FLOAT> cudnn_conv;
        cudnn_conv.compute_output_shape(inputs, cudnn_outputs, conv_param);
        cudnn_out_d.re_alloc(cudnn_outputs[0]->shape());
        cudnn_out_h.re_alloc(cudnn_outputs[0]->shape());
        {
            cudaDeviceSynchronize();
            cudnn_conv.init(inputs, cudnn_outputs, conv_param, SPECIFY, VENDER_IMPL, ctx1);
            cudaDeviceSynchronize();
            cudnn_conv(inputs, cudnn_outputs, conv_param, ctx1);
            cudaDeviceSynchronize();
            ...
            for (int n = 0; n < out_num; n++) {
                // foreach: relu()
                float cudnn_var = cudnn_out_h.mutable_data(0)[n];
                cudnn_out_h.mutable_data(0)[n] = cudnn_var >= 0.0f ? cudnn_var : 0.0f;
            }
        }
