...
void test_permute(std::vector<TensorType*>& inputs_big, std::vector<TensorType*>& outputs_big,
                  ...
                  PermuteParam<Tensor<NV, AK_FLOAT, NCHW>>& param, bool get_time, Context<NV>& ctx) {
    typedef Tensor<NV, AK_FLOAT, NCHW> TensorDf4;
    /*prepare data*/
    Permute<NV, AK_FLOAT, AK_FLOAT, AK_FLOAT, NCHW> permute;
    permute.compute_output_shape(inputs, outputs, param);
    ...
    permute.init(inputs, outputs, param, SPECIFY, SABER_IMPL, ctx);

    /*warm up*/
    permute(inputs, outputs, param, ctx);
    ...
    printf("shape: %d, %d, %d, %d\n", valid_shape[0],
           valid_shape[1], valid_shape[2], valid_shape[3]);
...
TEST(TestSaberFuncPowerNV, test_func_constructor) {
    ...
    std::vector<int> nhwc2nchw = {0, 2, 3, 1};
    PermuteParam<TensorDf4> param1(nhwc2nchw);
    PermuteParam<TensorDf4> param2(nchw2nhwc);
                    ...
                    for (auto output_share_sub : {
                        ...
                        for (auto get_time : {
                            ...
                            LOG(INFO) << input_share << "," << output_share << "," << input_share_sub << "," << output_share_sub
                                      << "," << get_time;
                            test_permute<TensorDf4>(inputs_big, outputs_big,
                                                    inputs, outputs,
