...
void test_crop(std::vector<TensorType*>& inputs_big, std::vector<TensorType*>& outputs_big,
               ...
               CropParam<Tensor<NV, AK_FLOAT, NCHW>>& param, bool get_time, Context<NV>& ctx) {
    typedef Tensor<NV, AK_FLOAT, NCHW> TensorDf4;
    /*prepare data*/
    Crop<NV, AK_FLOAT, AK_FLOAT, AK_FLOAT, NCHW> crop;
...
TEST(TestSaberFuncNV, test_func_constructor) {
    ...
    CropParam<TensorDf4> param(/* axis_in*/ 2, /*offset*/offset, /*shape_in*/shape);
                ...
                for (auto output_share_sub : {
                    ...
                    for (auto get_time : {
                        ...
                        LOG(INFO) << input_share << "," << output_share << "," << input_share_sub << "," << output_share_sub
                                  << "," << get_time;
                        test_crop<TensorDf4>(inputs_big, outputs_big,
                                             inputs, outputs,
