...
void test_conv_ab_test(int group,
                       ...
                       int stride_h, int stride_w, int dilation_h, int dilation_w,
                       int pad_h, int pad_w, bool bias_term,
                       SaberImplStrategy strategy, ImplEnum impl_type=SABER_IMPL) {
    ...
    output_dev.re_alloc(output_dev.valid_shape(), AK_FLOAT);
    typename Tensor<TargetType>::API::stream_t stream = ctx1.get_compute_stream();
    conv.init(input_v, output_v, param, strategy, impl_type, ctx1);
    ...
    for (int i = 0; i < 6; ++i) {
        ...
        if (max_ratio < 1e-4) {
            ...
            LOG(FATAL) << " FAIL in ab test!!! max_ratio = " << max_ratio << " max_diff = " << max_diff
                       ...
                       << " kernel_w = " << kernel_w
                       << " out_channels = " << out_channels
                       << " impl: " << ((impl_type == VENDER_IMPL) ? " VENDER " : " SABER");
...
TEST(TestSaberFunc, test_saber_conv_results_bm) {
    ...
    for (auto group : group_v) {
        ...
        test_conv_results<BM, X86>(group, input_num, in_channels,
                                      height, width, out_channels, kernel_h_w,
                                   kernel_h_w, stride_h, stride_w, dilation, dilation,
                                   pad_h_w, pad_h_w, bias_term, with_relu, SPECIFY, VENDER_IMPL);
    }
}
#endif
