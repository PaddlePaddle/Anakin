        ...
        for(int h_in : {2, 8, 32}){
            for(int ch_in : {2, 3, 8, 64}){
                for(int num_in:{1, 21, 32}){
                    ...
                    testbase.run_test(fc_cpu_base<float, NV, NVHX86>);
                }
            }
        }
    }
...
#ifdef USE_X86_PLACE
    TestSaberBase<X86, X86, AK_FLOAT, Fc, FcParam> testbase;
    
    Tensor<X86> weights_h;
        ...
        for(int h_in : {2, 8, 32}){
            for(int ch_in : {2, 3, 8, 64}){
                for(int num_in:{1, 21, 32}){
                    ...
                    weights_h.re_alloc(shape_w, AK_FLOAT);
                    fill_tensor_rand(weights_h, 0.1, 1.5);
                    FcParam<X86> param(&weights_h, out_num);
                    testbase.set_param(param);
                    testbase.set_rand_limit(1, 12);
                    testbase.set_input_shape(shape);
                    testbase.run_test(fc_cpu_base<float, X86, X86>);
                }
            }
        }
    }
