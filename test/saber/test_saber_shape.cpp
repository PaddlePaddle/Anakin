    ...
    for (int N = 1; N < 20; ++N) {
        for (int C = 1; C < 20; ++C) {
            for (int H = 1; H < 20; ++H) {
                for (int W = 1; W < 20; ++W) {
                    switch(layout_type) {
                        case Layout_invalid:   // Layout_invalid causes the unit test to fail.
                    ...
                    CHECK_EQ(sh1.canon_axis(-1), data.size() - 1);
                    CHECK_EQ(sh2.num(), N);
                    ...
                    CHECK_EQ(sh2.canon_axis(-1), data.size() - 1);
                    ...
                    CHECK_EQ(sh3.canon_axis(-1), data.size() - 1);
