...
static void gemm_naive(int m,int n,int k,const float alpha,const Dtype * a, const Dtype*b ,const float beta,Dtype *c,bool trans_a=false,bool trans_b=false){
    for(int i=0;i<m;i++){
        for(int j=0;j<n;j++){
            Dtype acc=0;
...
void gemm_transpose_conv(const std::vector<Tensor<TargetType_H>*>& inputs,
    ...
    for (int i = 0; i < num; ++i) {
        ...
        //! add bias
        if (bias_term) {
            fill_bias_relu(dout_batch, static_cast<const float*>(bias_host.data()), chout, wout * hout, with_relu);
        }
...
//    fill_tensor_const(weights_origin_host,1);
...
//        fill_tensor_const(weights_dev, 1);
        if (bias_term) {
        ...
        LOG(INFO)<<kernel_h<<","<<kernel_w<<","<<pad_h<<","<<pad_w<<","<<stride_h<<","<<stride_w<<", ["<<input_num<<","<<in_channels<<","<<height<<","<<width<<"] ,"<<out_channels<<",bias = "<<bias_term;
...
//    std::vector<int> stride_w_v{1, 2};
    ...
    std::vector<int> in_h_v{3,4};
    std::vector<int> in_w_v{3,4};
    std::vector<int> input_num_v{1};
    std::vector<int> input_channels_v{3,4};
    std::vector<int> output_channels_v{3,4};
    ...
    for (auto group : group_v) {
        ...
        if (bias_term) {
            bias_dev.re_alloc(bias_s, AK_FLOAT);
            fill_tensor_const(bias_dev, 1);
//            fill_tensor_rand(bias_dev, -2.f, 2.0f);
        }
        ConvParam<DEVICE> param_nv(group, pad_h, pad_w,
                               stride_h, stride_w,
                               dilation_h, dilation_w,
