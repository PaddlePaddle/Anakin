...
SaberStatus test_arm_sgemv(int M, int K, bool tra, bool flag_bias, bool flag_relu, int in_threads) {
    double to = 0;
...
//    print_tensor(tbias);
    ...
    int m = M;
    int k = K;
    LOG(INFO) << "sgemm M: " << m << ", K: " << k << ", transA: " << (tra? "true" : "false") \
...
//    LOG(INFO) << "compare result with basic sgemm: " << (COMPARE_RESULT? "true" : "false");
    ...
    const float* dbias = static_cast<const float*>(tbias.data());
    float* dc_basic = static_cast<float*>(tout_basic.mutable_data());
    if (COMPARE_RESULT) {
        LOG(INFO) << "run basic conv for precision comparation";
        basic_gemv(m, k, da, db, dbias, dc_basic, 1.f, 0.f, tra, flag_bias, flag_relu);
    ...
    for (int i = 0; i < test_iter; ++i) {
        t1.clear();
        t1.start();
        sgemv(da, db, dc_saber, false, m, k, flag_bias, dbias, flag_relu);
        t1.end();
...
TEST(TestSaberLite, test_func_sgemm_prepacked) {
    if (Basic_test) {
        ...
        for (auto& th : {1, 2, 4}) {
            SaberStatus flag = test_arm_sgemv(m, k, false, flag_bias, flag_relu, th);
...
TEST(TestSaberLite, test_func_sgemm_prepacked_custom) {
    auto flag = test_arm_sgemv(M, K, false, flag_bias, flag_relu, threads);
    if (flag != SaberSuccess) {
        LOG(FATAL) << "test m = " << M << ", k=" << K << \
