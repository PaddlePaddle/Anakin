...
TEST(NetTest, net_execute_base_test) {
    ...
    h_image.re_alloc(image_shape);
    h_image_info.re_alloc(image_info_shape);
    
    float* h_image_data = (float*)(h_image.mutable_data());
    float* h_image_info_data = (float*)(h_image_info.mutable_data());
    read_tensor_from_file(h_image_data, h_image.valid_size(), image_file.c_str());
    read_tensor_from_file(h_image_info_data, h_image_info.valid_size(), image_info_file.c_str());

    d_image->copy_from(h_image);
    d_image_info->copy_from(h_image_info);
    ...
    write_tensorfile(*net_executer.get_out_list()[1],"bbox_pred.tmp_1.txt");
    write_tensorfile(*net_executer.get_out_list()[2],"softmax_0.tmp_0.txt");
	//} // inner scope over
